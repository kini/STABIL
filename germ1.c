...
struct multiset {
	int c, a, l;
	struct triangle *joey;
	struct multiset *next, *prev;
};
struct bucket_triangle {
	struct triangle *first, *last;
};
struct bucket_multiset {
	struct multiset *first, *last;
};
struct color_class {
	int m;
	struct multiset *point;
} ewc[vm2];
...
struct multiset *sort_multisets(struct multiset *p, struct multiset *px)
{
	...
	for (xp = bucket; xp < bucket + hi; xp++) {
		...
		if (x < may) {
			...
			if (x) {
				newcolor++;
				zp = ewc[xp->first->a].point;
				ewc[newcolor].point = zp;
				ewc[newcolor].m = x;
				for (j = 0; j < x; j++) {
					zp->a = newcolor;
					zp = zp->next;
...
struct multiset *multisets(struct triangle *pt1, struct multiset *pm1)
{
...
struct multiset *presort(struct multiset *p)
{
	static struct bucket_multiset bucket[vm], *xp, *yp;
	struct color_class *pc;
	for (xp = bucket; xp < bucket + newcolor + 1; xp++)
		xp->last = 0;
	SCAN(p, p->a;
		 ewc[p->a].m++;
		);
...
struct triangle *triangles(struct multiset S[vm][vm])
{
	struct triangle help, *pt;
	struct color_class *pc;
	struct multiset *pm;
	int i, j, x, y, v;
	pt = &help;
...
void main(int narg, char *arg[10])
{
	...
	for (pc = ewc; pc < ewc + n * n; pc++)
		pc->m = 0;
	for (i = 0; i < n; ++i)
		ewc[S[i][i].a].m += (ewc[S[i][i].a].m) ? 0 : 1;
	for (i = 0; i < n; ++i)
		for (j = i + 1; j < n; ++j) {
			if (ewc[S[i][j].a].m == 1) {
				printf("please check your input!\n");
